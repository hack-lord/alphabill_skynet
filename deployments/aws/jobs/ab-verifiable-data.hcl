job "[[ .gt_environment ]]-ab-verifiable-data" {
    datacenters = ["[[ .nomad_datacenter ]]"]
    type = "service"

    constraint {
        attribute = "${meta.group}"
        value = "apps_data_partition"
    }
    ...
    group "app" {
        ...
        task "ab-verifiable-data" {
            ...
            env {
                GENESIS_PATH = "[[ .gt_environment ]]/rootchain/rootchain/partition-genesis-1.json"
                KEYS_PATH = "[[ .gt_environment ]]/vd${meta.index}/vd/keys.json"
            }
            ...
            template {
                data = "PEERS={{ key \"[[ .gt_environment ]]/vd_peers.txt\" }}"
